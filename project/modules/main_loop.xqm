...
declare variable $loop:vocabulary   := doc("/db/apps/mermeid/project/resources/site_xml/delius-keywords.xml");
...
declare variable $loop:collection   := "DCW";
...
{
  let $exclude := request:get-parameter("excludeAnthologies", "")
  let $result  := 
    if($loop:collection eq "DCW") then
...
{
  let $get_notbefore:= request:get-parameter("notbefore","1880")
  let $notbefore := 
  ...
  let $get_notafter := request:get-parameter("notafter","1931")
  let $notafter := 
...
:)
  let $docgenre1 := string-join($doc//m:workList/m:work/m:classification/m:termList/m:term/string(), " ")
  let $docgenre2 := string-join($doc//m:workList/m:work/m:classification/m:termList/m:term/string(), " ")
  let $occurrence :=
    if( string-length($genre)=0) then
      true()
    else if(contains($docgenre1,$genre) ) then
        true()
  ...
  $key as xs:string) as xs:string
{
  ...
  let $sort_key:=
    if($key eq "person") then
      replace(lower-case($doc//m:workList/m:work[@analog="frbr:work"]/m:titleStmt[1]/m:respStmt/m:persName[1]/string()),"\\\\ ","")
    else if($key eq "title") then
      replace(lower-case($doc//m:workList/m:work[@analog="frbr:work"]/m:titleStmt[1]/m:title[1]/string()),"\\\\ ","")
    else if($key eq "date") then
      let $dates := 
          for $date in $doc//m:workList
	    /m:work
            /m:history
	    /m:creation
