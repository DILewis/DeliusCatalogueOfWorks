...
<xsl:transform xmlns="http://www.music-encoding.org/ns/mei" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xl="http://www.w3.org/1999/xlink" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:m="http://www.music-encoding.org/ns/mei" xmlns:dcm="http://www.kb.dk/dcm" exclude-result-prefixes="m dcm h xl" version="1.0">
...
-->
  ...
  <!-- get external source description -->
    ...
    <!-- Ouch! directory name "public" hard-coded here - can we find some more generic solution? -->
        <xsl:variable name="doc_name" select="concat('http://',$hostname,'/dcm/cnw/data/',substring-before(@target,'#'))"/>
        ...
        <source>
            ...
            <xsl:comment> Source description imported from <xsl:value-of select="@target"/>
            </xsl:comment>
            <xsl:apply-templates select="$doc//m:meiHead/m:fileDesc/m:sourceDesc/m:source[@xml:id=$ext_id]/*[name()!='relationList']"/>
